...
Vue.component('super-selected', {
	...
	template: '\
		<div :id="id" class="easyUtil-dropdown easyUtil-blueForm" v-outsideclose="closeMenu" @click="openMenu()" :class="openClass">\
			<button type="button" class="easyUtil-dropdown-toggle easyUtil-flexContainerRow" data-toggle="easyUtil-dropdown">\
				<span class="">{{spanText}}</span>\
				<i class="easyUtil-fBlue easyUtil-downarray"></i>\
			</button>\
			<ul class="easyUtil-dropdown-menu easyUtil-dropdown-menu-right menu-list">\
				<li v-for="(value,i) in values">\
					<a class="" :val="value.val" @click="getVal(i)">{{value.text}}</a>\
				</li>\
			</ul>\
		</div>',
	data: function() {
	...
	methods: {
		...
		getVal: function(i) {
			...
			this.spanText = this.values[i].text;
			this.$emit("input", this.spanText);  //父组件通信
			this.$emit("val", val);//父组件通信
		}
	}
});
//  ========== 
